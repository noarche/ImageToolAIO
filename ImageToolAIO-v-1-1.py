...
if __name__ == "__main__":
    ...
    while True:
        ...
        keep_metadata = get_user_input("Keep metadata? (yes/no): ").lower() == 'yes'
        save_format = get_user_input("Enter the format to save images as (png, jpg, jpeg, webp): ").lower()
        compress = get_user_input("Compress images? (yes/no): ").lower() == 'yes'
        
        resize = get_user_input("Resize images? (yes/no): ").lower() == 'yes'
        resize_percent = 100
        if resize:
            resize_percent = float(get_user_input("Resize images by %: "))

        add_metadata = {}
        if not keep_metadata and get_user_input("Would you like to add metadata (author, keyword, copyright)? (yes/no): ").lower() == 'yes':
        ...
        if get_user_input("Would you like to restart the script? (yes/no): ").lower() != 'yes':
            break
